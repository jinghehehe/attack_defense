...
from utils2 import (weights_init, save_checkpoint, load_checkpoint, AverageMeter)
...
class UDenoiser:
    def __init__(self, args, device, image_nc, save_path, batch_per_epoch, train=True,
                 LPR=None,LPR2=None,LPR3=None):
        self.device = device
    ...
    def backward(self):
        ...
        if self.LPR and self.LPR2 and self.LPR3:
            self.loss_features = cfg.lam_l1 * (F.smooth_l1_loss(self.rc_features, self.gt_features)+F.smooth_l1_loss(self.rc_features2, self.gt_features2)+F.smooth_l1_loss(self.rc_features3, self.gt_features3))
        elif self.LPR:     # 特征层的重构损失
    ...
    def set_input(self, image, labels):
        ...
        if self.LPR:
            ...
            self.rc_features = self.LPR(self.rc_images)
            self.gt_features = self.LPR(self.gt_images)
        if self.LPR2:
            # self.rc_features = self.LPR(self.rc_images, True)
            # self.gt_features = self.LPR(self.gt_images, True)
            self.rc_features2 = self.LPR2(self.rc_images)
            self.gt_features2 = self.LPR2(self.gt_images)
        if self.LPR3:
            # self.rc_features = self.LPR(self.rc_images, True)
            # self.gt_features = self.LPR(self.gt_images, True)
            self.rc_features3 = self.LPR3(self.rc_images)
            self.gt_features3 = self.LPR3(self.gt_images)
